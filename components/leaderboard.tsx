  ...
  return (
    <div className="rounded-2xl border border-[rgba(214,176,93,.25)] p-3 bg-[color:rgb(13,27,30)]/60 flex items-center justify-between">
      ...
      <div className="flex items-center gap-3">
        <Badge
          className={cn(
            "text-white border border-[rgba(214,176,93,.35)] bg-gradient-to-br from-[rgba(21,177,167,.15)] to-[rgba(214,176,93,.12)] shadow-[0_0_12px_rgba(21,177,167,.35)]",
            badgeClass
          )}
        >
          {name}
        ...
        <div className="font-bold tabular-nums">
          {row.power_level.toLocaleString()}
        </div>
...
function TierBadge({ value, override }: { value: number; override?: string }) {
  const tier = override
    ? { name: override, badgeClass: "bg-emerald-600" }
    : tierFromPowerLevel(value);
  return (
    <Badge
      className={cn(
        "text-white border border-[rgba(214,176,93,.35)] bg-gradient-to-br from-[rgba(21,177,167,.15)] to-[rgba(214,176,93,.12)] shadow-[0_0_12px_rgba(21,177,167,.35)]",
...
function MetricsDialog({
  ...
  useEffect(() => {
    ...
    setError(null);
    getMetrics(puuid)
      .then((data) => {setRows(data); console.log(data)})
      ...
      <DialogContent className="max-w-5xl bg-[#10161a] text-white border border-[rgba(214,176,93,.25)]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <Info className="h-5 w-5 text-[#6b9ea6]" />
            Recent Match Metrics
          </DialogTitle>
          <DialogDescription>
            Per-match stats derived from Riot’s postgame data.
          </DialogDescription>
        </DialogHeader>
        {loading && (
                ...
                {rows.map((r) => (
                  <TableRow key={r.match_id}>
                    <TableCell className="font-mono text-xs">
                      {r.match_id.slice(0, 10)}…
                    </TableCell>
                    <TableCell>
                      <span
                        className={cn(
                          "font-medium",
                          r.win ? "text-emerald-600" : "text-rose-600"
                        )}
                      >
                        {r.win ? "Win" : "Loss"}
                      </span>
                      <div className="text-xs text-white/70">
                        {r.champion_name} · {secondsToClock(r.game_duration)}
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      ...
                      <span className="text-xs text-white/70">
                        (KDA {Number.parseFloat(r.kda).toFixed(2)})
                      </span>
                    </TableCell>
                    <TableCell>{r.gold_per_minute ?? "—"}</TableCell>
                    <TableCell>{r.cs_per_minute ? Number.parseFloat(r.cs_per_minute).toFixed(2) : "—"}</TableCell>
                    <TableCell>
                      {r.damage_per_minute ? Number.parseFloat(r.damage_per_minute).toFixed(2) : "—"}
                    </TableCell>
                    <TableCell>{r.kill_participation && formatPercent(Number.parseFloat(r.kill_participation))}</TableCell>
                    <TableCell>{r.vision_score_per_minute ?? "—"}</TableCell>
                    <TableCell>
                      {formatPercent(r.team_damage_percentage)} /{" "}
                      {formatPercent(r.damage_taken_on_team_percentage)}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
    ...
    <Card className="w-full bg-[#10161a]/90 border border-[rgba(214,176,93,.18)] shadow-[0_0_0_1px_rgba(214,176,93,.25),0_10px_30px_rgba(0,0,0,.45)]">
      <CardHeader className="text-white">
        <CardTitle className="flex items-center gap-2">
          <Trophy className="h-5 w-5 text-[#15b1a7]" />
          Leaderboard
        </CardTitle>
      ...
      <CardContent>
          ...
          <div className="overflow-x-auto">
            {pinned && (
              <div className="mb-3">
                <PinnedRow
                  row={pinned}
                  onOpen={() => setMetricsPuuid(pinned.puuid)}
                />
              </div>
            )}
            <Table className="w-full text-white/70 rounded-md">
              <TableHeader>
                <TableRow>
                  <TableHead className="text-white/70 w-16">#</TableHead>
                  <TableHead className="text-white/70 min-w-[160px]">Player</TableHead>
                  <TableHead className="text-white/70">Power Level</TableHead>
                  <TableHead className="text-white/70">Tier</TableHead>
                  <TableHead className="text-white/70 w-28 text-right">Actions</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {sorted.map((r, idx) => (
                  ...
                  >
                    ...
                    <TableCell className="font-semibold">
                      {r.power_level.toLocaleString()}
                    </TableCell>
                    <TableCell>
                      <TierBadge
                        value={r.power_level}
                        override={r.tier || undefined}
                      />
                    </TableCell>
                    <TableCell className="text-right">
                      <Button
                        ...
                        onClick={() => setMetricsPuuid(r.puuid)}
                      >
                        Metrics
                      </Button>
                    </TableCell>
        ...
        <MetricsDialog
          puuid={metricsPuuid}
          onOpenChange={() => setMetricsPuuid(null)}
        />
      </CardContent>
    </Card>
  );
